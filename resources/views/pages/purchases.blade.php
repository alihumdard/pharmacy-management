...
@section('content')
    ...
    <main class="overflow-y-auto p-4 md:p-8 min-h-[calc(100vh-70px)] bg-gray-50" 
            ...
            <div class="relative min-h-screen flex items-center justify-center p-4">
                <div class="relative bg-white w-full max-w-5xl rounded-2xl shadow-2xl overflow-hidden">
                    ...
                    <div class="p-4 border-t bg-gray-50 flex justify-end gap-3">
                        <button @click="showModal = false" class="px-6 py-2 bg-gray-200 rounded-lg font-bold text-gray-600">Discard</button>
                        <button onclick="submitFinalPO()" class="px-8 py-2 bg-green-600 text-white rounded-lg font-bold shadow-xl transition hover:bg-green-700 active:scale-95">Submit Order</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div id="productModal" class="fixed inset-0 bg-black/50 hidden justify-center items-center z-[50] p-4">
            <div id="productModalBox" class="bg-white w-full max-w-3xl rounded-2xl shadow-2xl flex flex-col overflow-hidden opacity-0 scale-95 transition-all duration-200">
                <div class="p-4 border-b bg-gray-50 flex justify-between items-center">
                    <h2 class="text-xl font-bold italic uppercase tracking-tighter">Product Details</h2>
                ...
                <div class="p-5 overflow-y-auto" style="max-height: 70vh;">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div class="col-span-2">
                            ...
                            <input id="p_name" placeholder="Enter product name..." class="w-full p-3 border rounded-xl mt-1 outline-none focus:ring-2 focus:ring-blue-400" />
                        </div>
                        <div>
                            <label class="text-xs font-bold uppercase text-gray-500 tracking-widest">Manufacturer</label>
                            <input id="p_manufacturer" placeholder="e.g. GSK, Getz" class="w-full p-3 border rounded-xl mt-1 outline-none focus:ring-2 focus:ring-blue-400" />
                        </div>
                        <div>
                            <label class="text-xs font-bold uppercase text-gray-500 tracking-widest">Category</label>
                            <select id="p_category" class="w-full p-3 border rounded-xl mt-1 outline-none focus:ring-2 focus:ring-blue-400">
                    ...
                    <div class="border p-4 rounded-xl bg-blue-50/20 border-blue-100">
                        <div class="flex justify-between items-center mb-3">
                            ...
                            <button type="button" onclick="openVariantModal()" class="px-3 py-1 bg-green-500 text-white text-xs font-bold rounded-lg shadow hover:bg-green-600 transition">
                ...
                <div class="p-4 border-t bg-gray-50 flex justify-end gap-3">
                    ...
                    <button onclick="addProductToPO()" class="px-5 py-2 bg-blue-600 text-white rounded-lg font-bold shadow-lg">Save Product</button>
                </div>
            </div>
        </div>

        <div id="variantModal" class="fixed inset-0 bg-black/70 hidden justify-center items-center z-[60] p-4">
            <div id="variantModalBox" class="bg-white w-full max-w-md rounded-2xl shadow-2xl opacity-0 scale-95 transition-all duration-200">
                <div class="p-4 border-b bg-gray-50 font-bold uppercase text-sm tracking-widest italic text-blue-600">Batch & SKU Details</div>
                <div class="p-5 grid grid-cols-2 gap-4">
                    <div class="col-span-2"><label class="text-xs font-bold uppercase text-gray-500">SKU / Label *</label><input id="v_sku" class="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-blue-400"></div>
                    <div><label class="text-xs font-bold uppercase text-gray-500">Batch No</label><input id="v_batch" class="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-blue-400"></div>
                    <div><label class="text-xs font-bold uppercase text-gray-500">Expiry Date</label><input id="v_expiry" type="date" class="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-blue-400"></div>
                    <div><label class="text-xs font-bold uppercase text-gray-500">Purchase (TP)</label><input id="v_tp" type="number" class="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-blue-400" value="0"></div>
                    <div><label class="text-xs font-bold uppercase text-gray-500">Stock Qty</label><input id="v_stock" type="number" class="w-full border p-2.5 rounded-xl outline-none focus:ring-2 focus:ring-blue-400" value="1"></div>
                </div>
    ...
    <script>
        ...
        window.addProductToPO = () => {
            ...
            poItemsList.push({ name: pName, manufacturer: pManufacturer, category: pCategory, variants: [...currentTempVariants], total: productTotal });
            updatePOTable();
            closeProductModal();
            ...
            try {
                ...
                poItemsList = po.items.map(item => ({
                    ...
                    variants: item.variants.map(v => ({
                        ...
                        tp: v.purchase_price,
                        stock: v.quantity,
                        subtotal: v.purchase_price * v.quantity
                    })),
